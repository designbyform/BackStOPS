...
// PDF pages — render all pages for vertical scroll
...
// Subtle radial pulse — replaces confetti
function Pulse({on}) {
  if (!on) return null;
  return (
    <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
      {[0,.12,.24].map((delay,i) => (
        <div key={i} style={{
          ...
          border:`1px solid rgba(29,29,31,${.18-i*.04})`,
          animation:`radPulse .9s cubic-bezier(0,0,.15,1) ${delay}s forwards`
        }}/>
      ))}
    </div>
  );
...
// Compact usage stat bar for studio header
...
export default function Marque() {
  ...
  // Track viewport for responsive nav
  useEffect(()=>{
  ...
  const processFile=useCallback(async file=>{
    ...
    setView("brand");setMenuOpen(false);
  ...
  const callAPI=async(userContent)=>{
    const response=await fetch("https://api.anthropic.com/v1/messages",{
      ...
      body:JSON.stringify({model:"claude-haiku-4-5",max_tokens:1000,messages:[{role:"user",content:userContent}]}),
    });
  ...
  const callAPIChat=async(msgs,system)=>{
    const body={model:"claude-haiku-4-5",max_tokens:1000,messages:msgs};
  ...
  const send=async content=>{
    ...
    const system=brandText
      ...
      : "You are Marque, an AI brand steward. No guidelines loaded — tell the user to upload their brand guide in the Brand tab.";

    // Always start with a user message — drop leading assistant context messages
...
{"title":"","format":"e.g. Twitter/X Banner","background":"#hex","accentColor":"#hex","textColor":"#hex","headline":"headline text","subtext":"tagline","visualElements":["el1","el2"],"mood":"word","artDirectionNotes":"2-3 sentences"}`;
  ...
  if(sharePreview){
    return <SharedGuideView brandName={brandName} pages={pdfPages} onBack={()=>setSharePreview(false)}/>;
  }
  ...
  return (
    <>
        ...
        <div onClick={()=>setMenuOpen(false)} style={{position:"fixed",inset:0,zIndex:400}}>
          <div onClick={e=>e.stopPropagation()} style={{position:"absolute",top:52,left:0,right:0,background:"rgba(245,245,247,.98)",backdropFilter:"blur(20px)",borderBottom:".5px solid rgba(0,0,0,.1)",paddingBottom:16,animation:"sUp .2s ease"}}>
            ...
            {plan==="seed"&&<div style={{padding:"14px 20px 0"}}>
              ...
              <div style={{height:3,background:"#f0f0f0",borderRadius:2,marginBottom:10}}><div style={{height:"100%",width:`${Math.min((msgCount/PLANS.seed.msgs)*100,100)}%`,background:msgCount>=PLANS.seed.msgs-5?"#ff3b30":"#1d1d1f",borderRadius:2}}/></div>
              ...
              <div style={{height:3,background:"#f0f0f0",borderRadius:2}}><div style={{height:"100%",width:`${Math.min((cCount/PLANS.seed.concepts)*100,100)}%`,background:"#1d1d1f",borderRadius:2}}/></div>
            </div>}
      ...
      <div style={{height:"100dvh",display:"flex",flexDirection:"column",overflow:"hidden",background:"#f5f5f7"}}>
        ...
        {/* Nav — desktop: inline tabs right; mobile/tablet: hamburger */}
        <nav style={{height:52,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 16px",background:"rgba(245,245,247,.95)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:".5px solid rgba(0,0,0,.12)",zIndex:300}}>
          ...
          {isDesktop?(
            <div style={{display:"flex",alignItems:"center",gap:2}}>
              ...
              {plan==="seed"&&(
                <button onClick={()=>setPaywall({reason:"Unlock unlimited access and AI image generation."})} style={{marginLeft:8,background:"#1d1d1f",border:"none",borderRadius:8,fontFamily:HN,fontSize:13,fontWeight:500,color:"#fff",padding:"6px 16px",cursor:"pointer"}}>Upgrade →</button>
              )}
            </div>
          ):(
        ...
        <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",minHeight:0}}>
                    ...
                    <div style={{background:"#fff",borderRadius:18,boxShadow:"0 4px 24px rgba(0,0,0,.08),0 0 0 .5px rgba(0,0,0,.05)",padding:"18px",marginBottom:14}}>
                      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <p style={{fontFamily:HN,fontSize:12,color:"#6e6e73"}}>{pdfPages.length?`${pdfPages.length} pages · PDF ready`:pdfRendering?"Rendering pages...":"Guidelines active"}</p>
                        </div>
                      ...
                      <div style={{display:"flex",gap:8}}>
                        ...
                        {isPaid&&pdfPages.length>0?(
                          <button onClick={()=>setShowShare(true)} style={{background:"#f5f5f7",border:"none",borderRadius:11,fontFamily:HN,fontSize:14,fontWeight:500,color:"#1d1d1f",padding:"13px 18px",cursor:"pointer",flexShrink:0}}>Share</button>
                        ):(
                          <button onClick={()=>setPaywall({reason:"Shareable brand guide links are available on Series ($29/mo) and Scale ($79/mo) plans."})} style={{background:"#f5f5f7",border:"none",borderRadius:11,fontFamily:HN,fontSize:13,fontWeight:500,color:"#aeaeb2",padding:"13px 14px",cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>Share ↑</button>
                        )}
                      </div>
                    </div>

                    {/* Full vertical PDF scroll */}
                    {(pdfPages.length>0||pdfRendering)&&(
                      <div style={{marginBottom:14}}>
                        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                          <p style={{fontFamily:HN,fontSize:12,fontWeight:600,color:"#1d1d1f"}}>Brand Guide</p>
          ...
          {view==="studio"&&(
            <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minHeight:0}}>
              {/* Studio header — mode toggle left, usage stats right */}
              <div style={{flexShrink:0,background:"#fff",borderBottom:".5px solid rgba(0,0,0,.08)",padding:"10px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                <div style={{display:"flex",background:"#f5f5f7",borderRadius:10,padding:3,gap:2,flexShrink:0}}>
                  {[{id:"text",label:"Text"},{id:"image",label:"Image"}].map(m=>(
                    <button key={m.id} onClick={()=>setMode(m.id)} style={{padding:"7px 18px",borderRadius:8,border:"none",background:mode===m.id?"#fff":"transparent",fontFamily:HN,fontSize:13,color:mode===m.id?"#1d1d1f":"#6e6e73",fontWeight:mode===m.id?500:400,boxShadow:mode===m.id?"0 1px 4px rgba(0,0,0,.08)":"none",transition:"all .15s",cursor:"pointer"}}>{m.label}</button>
                  ))}
                </div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:14,flexShrink:0}}>
                  {plan==="seed"?(
                    <>
                      <StatBar label="Msgs" used={msgCount} total={PLANS.seed.msgs}/>
                      <StatBar label="Ideas" used={cCount} total={PLANS.seed.concepts}/>
                      <StatBar label="Images" used={0} total={0} locked/>
                    </>
                  ):(
                    <>
                      <StatBar label="Msgs" used={0} total={1e9}/>
                      <StatBar label="Ideas" used={0} total={1e9}/>
                      <StatBar label="Images" used={imgCount} total={cur.imgs}/>
                    </>
                  )}
                  {plan==="seed"&&(
                    <button onClick={()=>setPaywall({reason:"Unlock unlimited messages, concepts, and AI image generation."})} style={{background:"#1d1d1f",border:"none",borderRadius:7,fontFamily:HN,fontSize:11,fontWeight:500,color:"#fff",padding:"5px 10px",cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>Upgrade</button>
                  )}
                </div>
              </div>
